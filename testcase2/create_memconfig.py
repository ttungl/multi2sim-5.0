...
def create_memconfig( 	num_of_cpu_cores, 	# number of CPU cores
						...
						gpu_model,			# [1] evergreen or [2] southern-islands
	...
	assert (gpu_model>0 & gpu_model<3), "Error! "
	...
	# GPU model
	if gpu_model ==1:
		type_of_gpu = "Evergreen"
	if gpu_model ==2:
		type_of_gpu = "SouthernIslands"
	...
	f.write(";; Cache Geometry for CPUs and %s GPUs.\n" % type_of_gpu);	
	f.write("\n");
	...
	f.write(";; Cache Geometry L2$ for %s GPUs.\n" % type_of_gpu);
	f.write("\n");
	...
	## L1-L2-of-GPU-cores
	f.write(";; %0.f L1$ for %s GPUs.\n" % ((num_of_gpu_cores/2), type_of_gpu));
	...
	f.write(";; %0.f L2$ for %s GPUs.\n" % ((num_of_gpu_cores/4), type_of_gpu));
	f.write("\n");
	...
	# GPU cores
	f.write(";; %0.f %s GPU cores.\n"  % (num_of_gpu_cores, type_of_gpu));
	f.write("\n");
	count_L1_gpu_inc1 = 0;
	L1_gpu_cache_inc1 = 0;
	...
	f.write(";; L1$-L2$ network for %s GPUs.\n" % type_of_gpu);
	f.write("\n");
	count_L2_cache_inc = 0;
	L2_cache_inc = 0;
	for i in range(num_of_gpu_cores):
